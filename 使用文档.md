...
项目现在支持完整的CLI命令行接口，专为自动化脚本和AI调用优化：
...
详细使用说明请查看：[CLI使用手册](./docs/CLI使用手册.md)
...
# 运行容器
docker run -d -p 5023:5023 -v $(pwd)/downloads:/app/downloads netease-music-api
```
...
├── uv.lock             # uv依赖锁定文件
├── requirements.txt     # Python依赖（兼容性保留）
└── README.md           # 项目说明
```
